...
const SocialsItem = defineAsyncComponent(() => import('./SocialsItem.vue'));
...
<div class="fixed top-0 bottom-0 right-0 w-full overflow-y-scroll bg-white-primary">
  <div class="flex items-center gap-16 mb-30">
    <button class="bg-yellow-primary flex items-center px-32 py-16 gap-5 rounded" role="button" @click="openModal">
      ...
      <p class="text-sm text-white-primary font-medium leading-21 uppercase hidden md:block">Ijtimoiy tarmoq qo‘shish</p>
    </button>
  ...
  <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-30">
    <socials-item v-for="item in items.list" :key="item.id" :id="item.id" :name="item.name" @edit="openModal" @remove="OPEN_DELETE_MODAL({ id: item.id, text: 'Diqqat, Ijtimoiy tarmoqni o‘chirishga aminmisiz?', title: `${item.name.uz}`, url: 'category', callback: getItems })"/>
  </div>
