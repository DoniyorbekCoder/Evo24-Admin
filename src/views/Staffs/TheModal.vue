        ...
        <form action="" @submit.prevent="submit" class="w-full space-y-30">
          <div class="grid md:grid-cols-2 gap-16">
            <div class="w-full relative">
              ...
              <label class="label font-medium transition-all text-sm inline leading-17 text-black-secondary absolute top-15 left-25">Ism</label>
            </div>
            <div class="w-full relative">
              <input type="text" class="border border-gray-secondary w-full bg-white-secondary text-sm leading-17 rounded py-16 px-21 placeholder:text-black-secondary focus:outline-none text-black-primary font-medium placeholder:font-normal" required />
              <label class="label font-medium transition-all text-sm inline leading-17 text-black-secondary absolute top-15 left-25">Parol</label>
            </div>
          </div>

          <div class="w-full h-1 bg-gray-secondary mt-32"></div>
          
          <div class="grid md:grid-cols-2 gap-16 my-36">
            <div class="flex items-center gap-4">
              <label class="switch">
                <input type="checkbox" :checked="data.formInfo.active" @click="data.formInfo.active = !data.formInfo.active">
                <span class="slider round"></span>
              </label>
              <p class="text-sm leading-20 font-medium text-black-primary">Ovqatlar</p>
            </div>
            <div class="flex items-center gap-4">
              <label class="switch">
                <input type="checkbox" :checked="data.formInfo.active" @click="data.formInfo.active = !data.formInfo.active">
