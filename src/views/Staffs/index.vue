...
const StaffItem = defineAsyncComponent(() => import('./StaffItem.vue'));
  ...
  <div class="fixed top-0 bottom-0 right-0 w-full overflow-y-scroll bg-white-primary">
    <div class="flex items-center gap-16 mb-30">
      <button class="bg-yellow-primary flex items-center px-32 py-16 gap-5 rounded" role="button" @click="openModal">
        ...
        <p class="text-sm text-white-primary font-medium leading-21 uppercase hidden md:block">Yangi xodim qo‘shish</p>
      </button>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-16">
      <staff-item v-for="item in items.list" :key="item.id" :id="item.id" :name="item.name" @edit="openModal" @remove="OPEN_DELETE_MODAL({ id: item.id, text: 'Diqqat, kategoriyani o‘chirishga aminmisiz?', title: `${item.name.uz}`, url: 'category', callback: getItems })" />
    </div>
