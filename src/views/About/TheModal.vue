         ...
         <div class="h-screen flex items-center justify-center fixed top-0 left-0 w-full">
            <div class="bg-white-primary p-20 md:p-32 shadow-lg rounded-16 z-0 space-y-16 w-570" :class="data.display ? 'z-0 scale-100 animate-blowUp' : ''">
               <div class="flex items-center gap-8">
                  <div @click="reset(data.formInfo); data.display = false"
                     class="cursor-pointer w-49 h-49 flex items-center justify-center rounded-full bg-white-secondary border border-gray-secondary">
               ...
               <base-image alt="photo" class="w-134 h-134 shrink-0 object-cover rounded-4 bg-white-primary border border-gray-secondary" />
               ...
               <form action="" @submit.prevent="submit" class="w-full space-y-32">
                  <div class="flex flex-col gap-16">
                     ...
                     <div class="flex flex-col gap-10">
                        ...
                        <div class="w-full relative" v-for="(item, index) in data.formInfo.name" :key="index" v-show="index == title.data.code">
                           <textarea class="border border-gray-secondary w-full bg-white-secondary text-sm leading-17 rounded py-16 px-21 placeholder:text-black-secondary focus:outline-none text-black-primary font-medium placeholder:font-normal resize-none" rows="10" required></textarea>
                           <label class="label font-medium transition-all text-sm inline leading-17 text-black-secondary absolute top-15 left-25">{{title.data.description}}</label>
                        </div>
                     </div>
                  </div>
