...
<div class="fixed top-0 bottom-0 right-0 w-full overflow-y-scroll bg-white-primary">
  ...
  <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-30">
    <category-item v-for="item in items.list" :key="item.id" :id="item.id" :name="item.name" :priority="item.priority" :active="item.active" :foods="item.foods" @edit="openModal" @remove="OPEN_DELETE_MODAL({ id: item.id, text: 'Diqqat, kategoriyani o‘chirishga aminmisiz?', title: `${item.name.uz}`, url: 'category', callback: getItems })"/>
  </div>
